...
const typeScale = [
  { size: '60px', weight: '500', label: 'Display / 60 medium', sample: 'Autonomous Systems' },
  { size: '40px', weight: '500', label: 'Heading / 40 medium', sample: 'Mission-ready at scale' },
  { size: '28px', weight: '500', label: 'Title / 28 medium', sample: 'Fleet operations & telemetry' },
  { size: '20px', weight: '500', label: 'Subhead / 20 medium', sample: 'One stack, from silicon to fleet' },
  { size: '16px', weight: '400', label: 'Body / 16 regular', sample: 'Encrypted datalinks, on-device autonomy and a hardened OS — engineered to operate where connectivity is contested.' },
  { size: '13px', weight: '400', label: 'Label / 13 regular', sample: 'FLEET STATUS · MUNICH · 8 ACTIVE · 14:22:09 UTC' },
  { size: '11px', weight: '500', label: 'Caption / 11 medium', sample: 'TELEMETRY · RSSI −82 dBm · BATTERY 86% · ALT 124m' },
];
</script>

<template>
  <div data-theme="dark" class="brand-root min-h-dvh bg-background text-foreground" :style="[accentStyle, bgStyle].filter(Boolean).join('; ')">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-0.5">
            ...
            <button
              ...
              :class="bg === b.k ? 'bg-secondary text-foreground' : 'text-muted-foreground hover:text-foreground'"
              @click="bg = b.k"
            >{{ b.l }}</button>
          </div>
    ...
    <div class="mx-auto max-w-5xl px-6 py-16 space-y-20">
      ...
      <section>
        ...
        <h2 class="mt-2 font-mono text-3xl font-medium">Color system</h2>
      ...
      <section>
        ...
        <h2 class="mt-2 font-mono text-3xl font-medium">Type scale</h2>
        <p class="mt-2 text-[14px] text-muted-foreground max-w-xl">
          Geist Mono is the primary voice across all surfaces — body, headings, data labels. No proportional companion at this stage.
        </p>

        <div class="mt-8 divide-y divide-border/60 border-y border-border/60">
          <div v-for="t in typeScale" :key="t.label" class="grid grid-cols-[180px_1fr] gap-4 py-5 items-baseline">
            <span class="font-mono text-[11px] text-muted-foreground self-start pt-1">{{ t.label }}</span>
            <span
              class="font-mono leading-tight"
              :style="`font-size: ${t.size}; font-weight: ${t.weight};`"
            >{{ t.sample }}</span>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <h2 class="mt-2 font-mono text-3xl font-medium">Interactive elements</h2>
        <p class="mt-2 text-[14px] text-muted-foreground max-w-xl">
          Brand accent surfaces in interactive controls — buttons, active states, focus rings. Everything else stays mono.
        </p>
      ...
      <section>
        ...
        <h2 class="mt-2 font-mono text-3xl font-medium">Above the fold</h2>
        <p class="mt-2 text-[14px] text-muted-foreground max-w-xl">
        ...
        <div class="mt-8 grid gap-6 lg:grid-cols-2">
          ...
          <div class="rounded-sm border border-border overflow-hidden">
            ...
            <div class="hero-grid p-8 relative min-h-64">
              <div class="relative z-10 max-w-lg">
                ...
                <h1 class="mt-4 font-mono text-4xl font-medium leading-[1.0] tracking-tight">
          ...
          <div class="rounded-sm border border-border overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-[1fr_200px] gap-6 items-center min-h-64">
              <div>
                <p class="font-mono text-[10px] font-medium uppercase tracking-[0.18em]" :style="dir !== 'mono' ? 'color: var(--brand)' : 'color: var(--muted-foreground)'">Auterion · 2026</p>
                <h1 class="mt-3 font-mono text-3xl font-medium leading-[1.02] tracking-tight">The operating system for autonomous robotics</h1>
          ...
          <div class="lg:col-span-2 rounded-sm border border-border overflow-hidden">
            ...
            <div class="relative mission-bg min-h-72 overflow-hidden">
              ...
              <svg class="absolute inset-0 h-full w-full" aria-hidden="true" preserveAspectRatio="none">
                ...
                <!-- mission path -->
                <path d="M 60 200 C 160 160, 200 100, 340 80 S 560 40, 640 28" fill="none"
                  stroke="var(--brand)"
              ...
              <div class="relative z-10 p-8 grid grid-cols-[1fr_auto] items-end h-72">
                <div class="self-center max-w-lg">
                  ...
                  <h1 class="mt-3 font-mono text-[42px] font-medium leading-[1.0] tracking-tight">
          ...
          <div class="border p-5 space-y-2"
            ...
            <ul class="space-y-1 mt-3">
              <li v-for="s in ['Uniquely owned — no competitors here','Strongest possible discipline signal','Ties marketing to the product register']" :key="s"
                class="flex items-start gap-2 font-mono text-[12px] text-muted-foreground">
              ...
              <li v-for="s in ['Zero warmth — may read cold to non-technical audiences','Harder to drive CTAs without a second color']" :key="s"
                class="flex items-start gap-2 font-mono text-[12px] text-muted-foreground/50">
                <span class="mt-0.5">−</span> {{ s }}
              </li>
            </ul>
          </div>
