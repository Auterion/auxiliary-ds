...
| **6.3 Defense layer** 🔧 | 6i | **In progress.** ✅ **`GuardedAction`** — hold/double/confirm guard for irreversible commands (arm/RTL/release), keyboard-equivalent + tap-proof, reduced-motion-safe progress, composes Button without modifying it. Still open: alert **model**, 508 / WCAG 2.2 AA + MIL-STD-1472 checklist, extended sunlight/night threshold gates, MGRS/unit-system formatting in `TelemetryValue` | XL |
