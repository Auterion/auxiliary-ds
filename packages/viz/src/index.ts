/**
 * `@auxiliary/viz` — the data-visualization layer.
 *
 * For now this is the **viz palette** surface: the categorical / sequential /
 * diverging scales, derived from the OKLCH primitive ramps in `@auxiliary/tokens`
 * and gated (ROADMAP §6.5 — `viz-palette.test.ts`) to stay clear of the reserved
 * status ladder and distinguishable under CVD. The chart set (sparkline, gauge,
 * streaming time-series, …) builds on this in later slices.
 *
