...
describe('NumberField', () => {
  ...
  it('maps each size prop to its recipe height class (defaulting to md)', () => {
    const cases: Array<['sm' | 'md' | 'lg', string]> = [
      ['sm', 'h-[max(var(--component-number-field-height-sm),var(--target-floor))]'],
      ['md', 'h-[max(var(--component-number-field-height-md),var(--target-floor))]'],
      ['lg', 'h-[max(var(--component-number-field-height-lg),var(--target-floor))]'],
    ];
    for (const [size, cls] of cases) {
      const wrapper = mount(NumberField, { props: { size } });
      expect(wrapper.classes()).toContain(cls);
    }
    expect(mount(NumberField).classes()).toContain('h-[max(var(--component-number-field-height-md),var(--target-floor))]');
  });
